...
const I18n = (() => {
  ...
  const TRANSLATIONS = {
    en: {
      ...
      "rules.swr": "Scenario-specific safe rate<br><small>Use Monte Carlo to test your withdrawal rate against your own return and volatility assumptions</small>",
      ...
      "result.mc_safe_wr": "MC-implied safe withdrawal rate",
      "result.mc_safe_wr_target": "success target:",
      "result.mc_safe_wr_chosen": "chosen:",
      ...
      "result.wr_budget": "WR Budget (funded lifestyle)",
      "result.expenses": "Annual Expenses",
      "result.pension": "Pension Income (net)",
      "result.nhi": "NHI Premium (with pension)",
      "result.nhi_gap": "NHI Premium (before pension)",
      "result.net_portfolio": "Net Portfolio Draw / yr",
      "result.mc_simulates_note": "This is what Monte Carlo actually simulates withdrawing — the same for every FIRE variant, since it comes from your portfolio and chosen withdrawal rate, not stated expenses.",
      "result.stated_spending": "Your Stated Spending",
      "result.deemed_wr": "Deemed Withdrawal Rate (pre-pension)",
      "result.deemed_wr_vs": "vs chosen",
      "result.cashflow_table": "Year-by-Year Cash Flow",
      "result.download_csv": "⬇ CSV",
      "result.show_table": "Show table",
      "result.hide_table": "Hide table",
      "cf.age": "Age",
      "cf.phase": "Phase",
      "cf.wr_budget": "WR Budget",
      "cf.pension": "Pension",
      "cf.nhi": "NHI",
      "cf.withdrawal": "From Portfolio",
      "cf.savings": "Savings",
      "cf.portfolio": "Portfolio",
      "cf.ideco": "iDeCo (locked)",
      ...
      "result.max_spend_before": "💰 Your WR budget (before pension)",
      ...
      "card.fire_number": "FIRE Number",
      "card.years_to_fire": "Years to FIRE",
      ...
      "compare.wr_budget": "WR Budget / yr",
      "compare.annual_expenses": "Stated Spending",
      "compare.pension": "Pension (net)",
      "compare.nhi": "NHI Premium",
      "compare.net_portfolio": "Net Portfolio Draw / yr",
      ...
      "compare.current_age": "Current Age",
      "compare.target_retirement_age": "Target Retirement Age",
      "compare.nenkin_claim_age": "Pension Claim Age",
      "compare.assumptions": "Assumptions",
      "compare.investment_return": "Investment Return (Accumulation)",
      "compare.retirement_return": "Investment Return (Retirement)",
      "compare.withdrawal_rate": "Withdrawal Rate (chosen)",
      "compare.deemed_wr": "Deemed WR (from actual spending)",
      "compare.expense_growth": "Retirement Expense Growth",
      ...
      "chart.mc.note":    "The <em>success rate</em> above measures whether your chosen withdrawal rate is sustainable — not whether your stated living expenses are covered. Check the deemed withdrawal rate in Retirement Cash Flow to see if your chosen rate actually funds your stated spending.",
      ...
      "help.withdrawal_rate_pct": "Your planned annual portfolio draw. The results page compares this with a Monte Carlo-derived safe rate for this scenario.",
      "help.japan_inflation_pct": "Recent Japan CPI has been ~2–3%. Reference only — this figure is not currently used in any calculation. Retirement expense growth (below) is what actually inflates your spending in the simulation.",
      "help.retirement_expense_growth_pct": "Typically below general inflation — retirees often spend less over time. This is the rate actually used to grow your expenses in retirement projections and Monte Carlo.",
      ...
      "help.spouse_income_jpy": "If your spouse earns ≤ ¥1,230,000/year you qualify for the full ¥380,000 spouse deduction (配偶者控除). Enter 0 if they have no income.",
    ...
    ja: {
      ...
      "rules.swr": "シナリオ別の安全率<br><small>リターンとボラティリティの前提に基づき、モンテカルロで取り崩し率を検証します</small>",
      ...
      "result.mc_safe_wr": "MC推定の安全な取り崩し率",
      "result.mc_safe_wr_target": "成功率目標:",
      "result.mc_safe_wr_chosen": "設定値:",
      ...
      "result.wr_budget": "取り崩し予算（生活費の原資）",
      "result.expenses": "年間支出",
      "result.pension": "年金収入（税引後）",
      "result.nhi": "国民健康保険料（年金受給後）",
      "result.nhi_gap": "国民健康保険料（年金受給前・軽減適用）",
      "result.net_portfolio": "ポートフォリオからの実際の引き出し額/年",
      "result.mc_simulates_note": "モンテカルロが実際にシミュレートしているのはこの金額です — ポートフォリオと選択した取り崩し率から算出されるため、どのFIRE戦略でも同じです（想定支出額には基づきません）。",
      "result.stated_spending": "想定支出額",
      "result.deemed_wr": "実質取り崩し率（年金受給前）",
      "result.deemed_wr_vs": "設定値との比較",
      "result.cashflow_table": "年別キャッシュフロー",
      "result.download_csv": "⬇ CSV",
      "result.show_table": "表を表示",
      "result.hide_table": "表を隠す",
      "cf.age": "年齢",
      "cf.phase": "フェーズ",
      "cf.wr_budget": "取り崩し予算",
      "cf.pension": "年金",
      ...
      "result.max_spend_before": "💰 取り崩し予算（年金前）",
      ...
      "card.fire_number": "FIRE目標額",
      "card.years_to_fire": "FIREまでの年数",
      "card.progress": "達成率",
      "card.mc": "MC成功率",
      ...
      "compare.wr_budget": "取り崩し予算/年",
      "compare.annual_expenses": "想定支出額",
      "compare.pension": "年金（税引後）",
      "compare.nhi": "国民健康保険料",
      "compare.net_portfolio": "ポートフォリオからの実際の引き出し額/年",
      ...
      "compare.profile": "プロフィール",
      "compare.current_age": "現在の年齢",
      "compare.target_retirement_age": "目標退職年齢",
      "compare.nenkin_claim_age": "年金受給開始年齢",
      "compare.assumptions": "前提条件",
      "compare.investment_return": "投資リターン（資産形成期）",
      "compare.retirement_return": "投資リターン（退職後）",
      "compare.withdrawal_rate": "取り崩し率（設定値）",
      "compare.deemed_wr": "実質取り崩し率（想定支出額から）",
      "compare.expense_growth": "退職後の支出増加率",
      ...
      "chart.mc.note":    "上の<em>成功率</em>は、選択した取り崩し率が持続可能かどうかを示すものであり、想定支出額が賄えるかどうかを示すものではありません。選択した取り崩し率が想定支出額を賄えるかどうかは、「退職後のキャッシュフロー」の実質取り崩し率で確認してください。",
      ...
      "help.withdrawal_rate_pct": "年間でポートフォリオから取り崩す予定の割合です。結果画面で、このシナリオのモンテカルロ推定安全率と比較します。",
      "help.japan_inflation_pct": "最近の日本のCPIは約2〜3%です。参考値のみ — この数値は現在どの計算にも使用されていません。実際に退職後の支出増加率として使われるのは下の「退職後の支出増加率」です。",
      "help.retirement_expense_growth_pct": "一般インフレより低いのが通常です。退職者は時間とともに支出が減る傾向があります。退職後のシミュレーションとモンテカルロで実際に使用される支出増加率です。",
