...
_FIELD_META: dict[str, dict] = {
    ...
    "mortgage_interest_rate_pct":            {"label": "Mortgage Interest Rate (%)",             "type": "float", "description": "Current annual interest rate (e.g. 0.7 for 0.7%). For variable rate loans, use the current applied rate.", "example": 0.7},
    "mortgage_type":                         {"label": "Mortgage Type",                          "type": "str",   "description": "variable or fixed",                                                                "example": "variable"},
    "mortgage_remaining_years":              {"label": "Mortgage Remaining Years",               "type": "int",   "description": "Years left on the loan",                                                          "example": 25},
    "mortgage_tax_credit_remaining_years":   {"label": "Housing Loan Tax Credit Years Left",    "type": "int",   "description": "住宅ローン控除 years remaining (0 if expired or not applicable)",                  "example": 0},
    "mortgage_tax_credit_principal_cap_jpy": {"label": "Tax Credit Principal Cap (JPY)",        "type": "int",   "description": "Maximum principal eligible for 住宅ローン控除 (¥30M standard, up to ¥50M for 認定長期優良住宅)", "example": 30_000_000},
    "mortgage_tax_credit_rate_pct":          {"label": "Tax Credit Rate (%)",                   "type": "float", "description": "Annual credit rate applied to mortgage balance (0.7% from 2024)",                  "example": 0.7},
    "mortgage_prepayment_fee_jpy":           {"label": "Prepayment Fee (JPY)",                  "type": "int",   "description": "One-time fee charged on lump-sum early repayment (0 if no fee)",                  "example": 0},
...
def _profile_from_form(form) -> FinancialProfile:
    ...
    return FinancialProfile(
        ...
        mortgage_interest_rate_pct=f("mortgage_interest_rate_pct", 0.7),
        mortgage_type=s("mortgage_type", "variable") if s("mortgage_type", "variable") in ("variable", "fixed") else "variable",
        mortgage_remaining_years=i("mortgage_remaining_years", 25),
        mortgage_tax_credit_remaining_years=i("mortgage_tax_credit_remaining_years", 0),
        mortgage_tax_credit_principal_cap_jpy=i("mortgage_tax_credit_principal_cap_jpy", 30_000_000),
        mortgage_tax_credit_rate_pct=f("mortgage_tax_credit_rate_pct", 0.7),
        mortgage_prepayment_fee_jpy=i("mortgage_prepayment_fee_jpy", 0),
